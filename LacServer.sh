...
if ! sudo -l &>/dev/null; then
    echo "У вас нет прав sudo, установка невозможна"
    exit 1 
fi
...
sudo mkdir -p /$USERNAME/.config/unity3d/MA/LAC
sudo mkdir -p /$USERNAME/.config/unity3d/MA/editor

echo 'Создание конфигурации'
sudo bash -c 'cat > /$USERNAME/.config/unity3d/MA/LAC/ServerConfig.txt << EOF
# Стандартные карты:
